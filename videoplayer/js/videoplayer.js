class VideoPlayer {
    ...
    init() {
        //Првоерить добавлено ли видео URL
    ...
    togglePlay() {
        const method = this.__video.paused ? 'play' : 
        'pause';
    ...
    __addEvents() {
        ...
        this.__skipBtns.addEventListener('click', (e) => {
          let target = e.target;
          
          if(target.classList.contains('skip__btn')){
            const skip = Number(target.dataset.skip);

            this.__video.currentTime += skip;
          } 
        });

        this.__volumeBar.addEventListener('input', () => this.__video.volume = this.__volumeBar.value);

        this.__playbackBar.addEventListener('input', () => this.__video.playbackRate = this.__playbackBar.value);
    }
        ...
        <div class="player">
          ...
          <div class="player__controls">
            ...
            <div class="skip__group">
                <button data-skip="${this.__settings.skipBack}" class="player__button skip__btn">« 1s</button>
                <button data-skip="${this.__settings.skipNext}" class="player__button skip__btn">1s »</button>
            </div>
          </div>
        </div>
        `;
