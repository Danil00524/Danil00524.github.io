    ...
    <v-dialog v-model="statusPopup" max-width="520px">
      <div class="module__thanks">
        <span
          ...
          @click.prevent="$store.commit('profiles/SET_CHANGED_PROFILE_CLOSE')"
        ></span>
        <div class="module__thanks-inner">
          <div class="module__thanks-title">
            {{
              $route.query.recoverySuccess === 'true'
                ? staticText.popups[2][actualLang.index]
                : staticText.popups[3][actualLang.index]
            }}
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      statusPopup: (state) => state.profiles.editPage.statusPopupSuccess,
    }),
  },
};
</script>
