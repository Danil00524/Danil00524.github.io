    ...
    <v-dialog v-model="dialog" max-width="520px">
      <div class="module__thanks">
        <span
          ...
          @click.prevent="successRecovery"
        ></span>
        <div class="module__thanks-inner">
          <div class="module__thanks-title">
            {{ staticText.popups.recoverySuccess.popupTitle[actualLang.index] }}
          </div>
          <div class="module__thanks-subtitle">
            {{ staticText.popups.recoverySuccess.popupText[actualLang.index] }}
          </div>
        </div>
      </div>
    </v-dialog>
...
export default {
  ...
  methods: {
    successRecovery() {
      if (!this.dialog) {
        this.$store.commit('profiles/SET_CHANGED_PROFILE_CLOSE');
      }
